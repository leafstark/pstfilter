...
| Matching | `src/matching/` | Keyword parsing, normalization, independent multi-keyword matching. |
...
keyword independently, routed to zero or more output streams, then released.
...
- **`OutputManager`** — one logical writer per keyword. For the expected
  <100-keyword range, persistent streams are used (no premature LRU pooling).
...
- **Fatal** (immediate non-zero exit): cannot open PST, unsupported format,
  cannot create output dir, no keywords, invalid regex.
